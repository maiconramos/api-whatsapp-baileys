...
console.log('%cmade with <3 by afarhansib and translated by maiconramos', 'font-weight: bold; font-size: 30px;color: #ff2600; text-shadow: 1px 1px 0px black , -1px -1px 0px white')
		  ...
		  promise = promise.then(function () {
		    ...
		    fetch(document.getElementById("server").value + "send-message", {
				...
				.then(res => {
				  ...
				  if (res.status) {
				  	updateStatus(i - 1, "Enviou")
				  } else {
				  	updateStatus(i - 1, "Falhou")
				  	toast("error! verifique novamente os dados", "danger")
				  }
				}).catch(err => {
